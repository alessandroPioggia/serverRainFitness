...
app.get('/', (req, res) => {
    res.json({
        status: 'ok',
        message: 'Rain Fitness API Server',
        version: '1.0.0'
    });
});
...
const io = new Server(server, {
    cors: {
        origin: ['https://rainfitness.netlify.app', 'http://localhost:3000'],
        methods: ['GET', 'POST'],
        credentials: true,
        allowedHeaders: ['Content-Type', 'Authorization', 'x-auth-token']
    },
...
io.on('connection', (socket) => {
    ...
    socket.onAny((event, ...args) => {
        console.log(`Evento Socket.IO ricevuto: ${event}`, args);
    });
